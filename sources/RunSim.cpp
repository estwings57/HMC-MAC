...
#ifdef HMC_MAC
void parseTraceFileLine(string &line, uint64_t &clockCycle, uint64_t &addr, TransactionType &tranType, unsigned &dataSize, unsigned &macSize)
#else
...
#endif
{
...
#ifdef HMC_MAC
	//HMC_MAC atomic
	// X : fixed point / L : floating point
	else if(tempStr.compare("MAC_X4B")==0)	tranType = MAC_X4B;
	else if(tempStr.compare("MAC_X8B")==0)	tranType = MAC_X8B;
	else if(tempStr.compare("MAC_X16B")==0)	tranType = MAC_X16B;
	else if(tempStr.compare("MAC_L4B")==0)	tranType = MAC_L4B;
	else if(tempStr.compare("MAC_L8B")==0)	tranType = MAC_L8B;
	else if(tempStr.compare("MAC_L16B")==0)	tranType = MAC_L16B;
#endif
	else {
	...
	spaceIndex = line.find_first_not_of(" ", previousIndex);
#ifdef HMC_MAC
	if(tranType >= MAC_X4B && tranType <= MAC_L16B) {
		if(spaceIndex==-1) {
			ERROR("  == Error - MAC atomic sholud have the number of multiplication");
			return;
		}
		tempStr = line.substr(spaceIndex, line.find_first_of(" ", spaceIndex) - spaceIndex);
...
#ifdef HMC_MAC
						unsigned macSize;
						parseTraceFileLine(line, issueClock, addr, tranType, dataSize, macSize);
#else
