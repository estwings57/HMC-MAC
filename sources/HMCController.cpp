...
#ifdef HMC_MAC
		case MAC_X4B:		cmdtype = MX4B;		mac = true;	reqDataSize = 32;	packetLength = 1 + ceil(tran->macSize/(double)4.0);	break;	//one operand(of multiplication) size is 4-Byte (1/4 of one FLIT) 
		case MAC_X8B:		cmdtype = MX8B;		mac = true;	reqDataSize = 32;	packetLength = 1 + ceil(tran->macSize/(double)4.0);	break;
		case MAC_X16B:		cmdtype = MX16B;	mac = true;	reqDataSize = 32;	packetLength = 1 + ceil(tran->macSize/(double)4.0);	break;
		case MAC_L4B:		cmdtype = ML4B;		mac = true;	reqDataSize = 32;	packetLength = 1 + ceil(tran->macSize/(double)4.0);	break;
		case MAC_L8B:		cmdtype = ML8B;		mac = true;	reqDataSize = 32;	packetLength = 1 + ceil(tran->macSize/(double)4.0);	break;
		case MAC_L16B:		cmdtype = ML16B;	mac = true;	reqDataSize = 32;	packetLength = 1 + ceil(tran->macSize/(double)4.0);	break;
#endif
...
#ifdef HMC_MAC
	newPacket->macSize = tran->macSize;
	newPacket->pktMAC = mac;
#endif
	return newPacket;
}
